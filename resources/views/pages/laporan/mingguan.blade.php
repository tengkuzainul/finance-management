    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-gray-600 mt-1">Laporan transaksi mingguan semua cabang</p>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('master-data.laporan-keuangan.export-pdf', ['tanggal_mulai' => $startOfWeek->format('Y-m-d'), 'tanggal_akhir' => $endOfWeek->format('Y-m-d')]) }}"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
            <form action="{{ route('laporan.mingguan') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($groupedByCabang as $cabangId => $items)
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500">{{ $cabang->alamat_lengkap ?? '-' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td
                                        class="px-6 py-4 text-sm text-right {{ $saldo >= 0 ? 'text-blue-600' : 'text-orange-600' }} font-medium">
                                        Rp {{ number_format($saldo, 0, ',', '.') }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-center">{{ $items->count() }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        <!-- Daily Breakdown -->
        @forelse($groupedByTanggal as $date => $dailyLaporans)
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
